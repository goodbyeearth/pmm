...
def compute_old_conv(input=None,old_params=None,scop=None,n_fil=None,**kwargs):
    ...
    for n in range(len(old_params)):
        ...
        old_c = activ(my_conv(input[n], 'old_'+scop+str(n), n_filters=n_fil, filter_size=3, stride=1, init_scale=np.sqrt(2),
                              ww=param[scop+'/w'], bb=param[scop+'/b'], **kwargs))
        old_conv.append(old_c)
        print()
    flag = True
    for c in old_conv:
        if flag:
...
def compute_old_linear(input=None,old_params=None,scop=None,n_hid=None,**kwargs):
    activ = tf.nn.relu
    old_linear = []
    for n in range(len(old_params)):
        param = old_params[n]
        print("Use old model/"+scop+"/w & b")
        old_l = activ(my_linear(input[n], 'old_'+scop+str(n), n_hidden=n_hid, init_scale=np.sqrt(2),
...
def custom_cnn_pgn(scaled_images, old_params=None, **kwargs):
    ...
    for n in range(len(old_params)):
        ...
        old_c = activ(my_conv(scaled_images, 'old_c1'+str(n), n_filters=16, filter_size=3, stride=1, init_scale=np.sqrt(2),
    ...
    old_linear,suml = compute_old_linear(input=old_conv,n_hid=256,scop='fc1',old_params=old_params)
    # old_linear = []
    # for n in range(len(old_params)):
    #     param = old_params[n]
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, old_params=None, **kwargs):
        ...
        with tf.variable_scope('model', reuse=reuse):
            ...
            if old_params:
                ...
                print()
                """CNN提取后的特征"""
                extracted_features, old_fc1, sum4 = custom_cnn_pgn(self.processed_obs, old_params=old_params, **kwargs)
                extracted_features = tf.layers.flatten(extracted_features)
                for fc in range(len(old_fc1)):
                    old_fc1[fc] = tf.layers.flatten(old_fc1[fc])
                sum4 = tf.layers.flatten(sum4)

                pi_h = extracted_features
                # TODO: 调
                layer_size = 64
                pi_h = activ(linear(tf.add(pi_h, sum4),'pi_fc0', n_hidden=layer_size))
                old_pi_fc0 = []
                for n in range(len(old_params)):
                    param = old_params[n]
                    print('Use old model/pi_fc0/w & b')
                    old_l = activ(my_linear(old_fc1[n],'old_pi_fc0'+str(n),n_hidden=layer_size,ww=param['pi_fc0/w'], bb=param['pi_fc0/b']))
                ...
                for n in range(len(old_params)):
                    ...
                    old_l = activ(my_linear(old_pi_fc0[n], 'old_pi_fc1'+str(n), n_hidden=layer_size, ww=param['pi_fc1/w'],
                                  bb=param['pi_fc1/b']))
                ...
                for i, layer_size in enumerate([64]):
                    vf_h = activ(linear(tf.add(vf_h, sum4), 'vf_fc' + str(i),n_hidden=layer_size))
                    for n in range(len(old_params)):
                        param = old_params[n]
                        print('Use old model/vf_fc0/w & b')
                        old_l = activ(my_linear(old_fc1[n], 'old_vf_fc0'+str(n), n_hidden=layer_size, ww=param['vf_fc0/w'],
                                      bb=param['vf_fc0/b']))
                ...
                value_fn = linear(tf.add(vf_h, sum_vf_fc),'vf' ,n_hidden=1)
                old_vf = []
                for n in range(len(old_params)):
                    param = old_params[n]
