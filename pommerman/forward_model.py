...
class ForwardModel(object):
    ...
    # @staticmethod
    # # 只推送有炸弹的情况
    # def get_rewards1(agents, game_type, step_count, max_steps, train_idx=0):
    #
    #     def any_lst_equal(lst, values):
    #         '''Checks if list are equal'''
    #         return any([lst == v for v in values])
    ...
    #         # 敌人胜利
    #         return [-1, 1, -1, 1]
    #     elif step_count >= max_steps and (
    #             any_lst_equal(alive_agents, [[0, 2, 1], [0, 1]]) or any_lst_equal(alive_agents, [[0, 2, 3], [0, 3]])):
    #         # 最大步数平局并且杀死了一个敌人
    #         return [0.4] * 4
    #     elif step_count >= max_steps:
    #         # 最大步数平局
    ...
    @staticmethod
    # 只推送有敌人出现的情况 --> 必须杀敌
    def get_rewards2(agents, game_type, step_count, max_steps, train_idx=0):
