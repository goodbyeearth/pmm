...
class Pomme(gym.Env):
    ...
    def _set_observation_space(self):
        ...
        min_obs = np.stack(np.array([[[0]*self._board_size]*self._board_size]*18),axis=2)
        max_obs = np.stack(np.array([[[9]*self._board_size]*self._board_size]*18),axis=2)
    ...
    def act(self,):
        ...
        self.act_step = self.model.act(agents, self.obs_step, self.action_space)
        return np.array(self.act_step[3])
    ...
    def get_observations(self):
        ...
        self.obs_step = self.observations
        observation_3 = featurize(self.observations[3])

        return observation_3 #self.observations
    ...
    def step(self, act_3):
        self.act()
        self.act_step[3]=int(act_3)
        self._intended_actions = self.act_step
