...
class SimpleNoBombAgent(BaseAgent):
    """This is a baseline agent. After you can beat it, submit your agent to
    compete.
    """

    def __init__(self, *args, **kwargs):
        super(SimpleNoBombAgent, self).__init__(*args, **kwargs)
    ...
    def act(self, obs, action_space):
        ...
        bombs = convert_bombs(np.array(obs['bomb_blast_strength']), bomb_life)
        ...
        if unsafe_directions:
            directions = self._find_safe_directions(
                board, my_position, unsafe_directions, bombs, enemies,items)
    ...
    def _find_safe_directions(self, board, my_position, unsafe_directions,
        ...
        for row, col in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if direction in unsafe_directions:
                ...
                if not utility.position_is_passable(board, position,
                                            enemies):
                    disallowed.append(direction)
            ...
            if utility.position_is_passable(board, position,
                ...
                for bomb in bombs:
                    if bomb['bomb_life'] == 1:
                        ...
                        if bomb_x == position[0] and abs(bomb_y - position[1]) <= bomb['blast_strength']:
                            #remove the direction
                            safe.pop()
                            break
                        elif bomb_y == position[1] and abs(bomb_x - position[0]) <= bomb['blast_strength']:
                            safe.pop()
                            break
        if not safe:
    ...
    def _maybe_bomb(ammo, blast_strength, items, dist, my_position):
        ...
        #zzw加上的内容
        if items.get(constants.Item.Passage) != None:
        #zzw加上的内容
