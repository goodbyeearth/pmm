...
class A2C(ActorCriticRLModel):
    ...
    def __init__(self, policy, gamma=0.99, n_steps=5, vf_coef=0.25, ent_coef=0.01, max_grad_norm=0.5,
                 learning_rate=7e-4, alpha=0.99, epsilon=1e-5, lr_schedule='constant', verbose=0, tensorboard_log=None,
                 _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False):

        super(A2C, self).__init__(policy=policy, verbose=verbose, requires_vec_env=True,
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("train_model", reuse=True,
                    ...
                    train_model = self.policy(self.sess, self.observation_space, self.action_space, self.n_envs,
                                              self.n_steps, n_batch_train, reuse=True, old_params=self.old_params, **self.policy_kwargs)
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="A2C",
              reset_num_timesteps=True,env=None, learning_rate=7e-4, n_steps=128, gamma=0.99):
        # temp params
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in tqdm(range(1, total_timesteps // self.n_batch + 1)):
